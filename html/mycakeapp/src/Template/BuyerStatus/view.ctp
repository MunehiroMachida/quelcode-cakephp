<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\BuyerStatus $buyerStatus
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Buyer Status'), ['action' => 'edit', $buyerStatus->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Buyer Status'), ['action' => 'delete', $buyerStatus->id], ['confirm' => __('Are you sure you want to delete # {0}?', $buyerStatus->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Buyer Status'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Buyer Status'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="buyerStatus view large-9 medium-8 columns content">
    <h3><?= h($buyerStatus->name) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Name') ?></th>
            <td><?= h($buyerStatus->name) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Phone Number') ?></th>
            <td><?= h($buyerStatus->phone_number) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Address') ?></th>
            <td><?= h($buyerStatus->address) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($buyerStatus->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Buyer Id') ?></th>
            <td><?= $this->Number->format($buyerStatus->buyer_id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Is Received') ?></th>
            <td><?= $buyerStatus->is_received ? __('Yes') : __('No'); ?></td>
        </tr>
    </table>
</div>
